...
export default function TodoItem({ todo }: { todo: any }) {
    ...
    return (
        <div className={`p-4 flex items-start gap-4 transition-all ${isPending ? "opacity-50" : ""} hover:bg-white/[0.02]`}>
            <button
                onClick={handleToggle}
                disabled={isPending}
                className="mt-1 flex-shrink-0 text-gray-400 hover:text-indigo-400 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded"
            >
                {todo.isCompleted ? (
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <p className={`text-sm font-medium ${todo.isCompleted ? "text-gray-500 line-through" : "text-gray-200"}`}>
                        {todo.task}
                    </p>
                ...
                <div className="flex items-center gap-3 text-[11px] text-gray-500">
                    <span className="flex items-center gap-1">
                        <span className="font-medium text-gray-400">{todo.createdBy}</span>
                    </span>
                    <span className="flex items-center gap-1">
